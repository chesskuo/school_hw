...
void DRV_SysXtal(U8 u8XtalMode)
{
		...
		//-----External Crystal
		//-----24MHz
		OUTW(rSYS_OPTION1,((INW(rSYS_OPTION1)&0xFFFF00FF) | 0x0012)); //Use HXTAL and divide 2
		//-----Crystal 12MHz
...
unsigned int time = 450000;

void short_()
{
	GPIO_PTB_GPIO = 0xFFC3; // .
	delay1(time*0.9);
...
int main()
{
	...
	short_();
	long_();
	GPIO_PTB_GPIO = 0x0000; // all LED
	delay1(time*3);
	GPIO_PTB_GPIO = 0xFFFF;
	delay1(time*3);

	unsigned int tmp = 0xFFFE;
	...
	// F
	short_(); short_(); long_(); short_();
	delay1(time*3);

	// J
	short_(); long_(); long_(); long_();
	delay1(time*3);
	...
	// C
	long_(); short_(); long_(); short_();
	delay1(time*3);

	// S
	short_(); short_(); short_();
	delay1(time*3);
